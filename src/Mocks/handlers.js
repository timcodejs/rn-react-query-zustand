...
export const handlers = [
  // 할일 목록
  rest.get('http://localhost:8081/todos', () => {
    console.log('get');
    return HttpResponse.json(Array.from(data.todos.values()));
  }),

  // 할일 추가
  rest.post('http://localhost:8081/todos', async ({request}) => {
    console.log('post');
  ...
  // 할일 삭제
  rest.delete('http://localhost:8081/todos/:id', ({params}) => {
    const {id} = params;
  ...
  // 할일 수정
  rest.put('http://localhost:8081/todos/:id', async ({request, params}) => {
    const {id} = params;
